Feature: Run to complete
  ...
  Scenario Outline: a replayed local operation advances the epoch exactly once
    ...
    Examples:
      ...
      | supersession           | new PlanHead generation             |
      | stale-close compensation | close attempt's completion_operation_key |
  ...
  Scenario: an unattributed ticket in a multi-target project pauses completion
    ...
    And only a parent verified as the epic's descendant adopts, recording external cascade evidence
    And one outside the epic executes its explicit-reopen obligation and routes to conflicting — the reclaim lifecycle must reparent the ticket beneath the epic before the row can resolve
  ...
  Scenario: a reopened ambiguity never replays a stale resolution
    ...
    Given a resolver from before the reopen completes late and attempts to link its advance, stamp the row, or transition it to adopted or conflicting
    Then every such generation-conditional write fails — the row's generation moved — and it mutates nothing, its state, targets, and errors untouched
