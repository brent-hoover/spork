...
func TestRecoveryStepsRunInDeclaredOrder(t *testing.T) {
	...
	steps := recoverySteps(intakerForTest(db), workspaceManagerForTest(t), bridgeForTest(t), loopForTest(db), submitterForTest(db), queueForTest(t, db), completerForTest(t, db), "actor-1", planner.Claimer{}, noTargets, noWork, noWork, "/target",
...
func TestReviewRecoverySurfacesRatherThanBlocks(t *testing.T) {
	...
	steps := recoverySteps(intakerForTest(db), workspaceManagerForTest(t), bridgeWithStore(db), loopForTest(db), submitterForTest(db), queueForTest(t, db), completerForTest(t, db), "actor-1", planner.Claimer{}, noTargets, noWork, noWork, "/target",
		orchestrator.Researcher{}, noFindingProject)
	for _, s := range steps {
...
func TestWithoutARepositoryOnlyTheTrackerStageRecovers(t *testing.T) {
	...
	all := recoverySteps(planner.Intaker{}, workspace.Manager{}, reviewbridge.Bridge{},
		...
		orchestrator.Completer{}, "actor-1", planner.Claimer{}, noTargets, noWork, noWork, "/target",
		orchestrator.Researcher{}, noFindingProject)
	if len(all) < 2 {
