Feature: Closing requires an approved review
  ...
  Scenario: a stale revision cannot authorize a close
    ...
    When SUT-7 is transitioned to "complete" naming that review at revision 1 with its approval's verdict event
  ...
  Scenario: a reversed approval cannot authorize a close
    ...
    When SUT-2 is transitioned to "complete" naming that review at its last approved revision with its latest verdict event
  ...
  Scenario: a merge-consumed approval still closes its issue
    ...
    When SUT-3 is transitioned to "complete" naming that review at its consumed revision with its approval's verdict event
  ...
  Scenario: a spent review cannot close a reopened issue
    ...
    When SUT-4 is transitioned to "complete" naming the same review at its approved revision with its approval's verdict event
    Then the transition is rejected — the review is already close-used
    Given a fresh review of SUT-4 is approved
    When SUT-4 is transitioned to "complete" naming the fresh review at its approved revision with its approval's verdict event
    Then the transition succeeds
  ...
  Scenario: naming the wrong review cannot close
    ...
    When SUT-5 is transitioned to "complete" naming its changes-requested review at its current revision with its latest verdict event
    Then the transition is rejected
    When SUT-5 is transitioned to "complete" naming SUT-6's review at its approved revision with its approval's verdict event
  ...
  Scenario: doc deliverables gate like code
    ...
    When SUT-2 is transitioned to "complete" naming that review at its approved revision with its approval's verdict event
    Then the transition succeeds
