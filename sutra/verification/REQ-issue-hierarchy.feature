Feature: Issue hierarchy
  ...
  Scenario: open children hold the parent open
    ...
    When SUT-1 is transitioned to "complete" naming its approved review at its current revision with its approval's verdict event
    ...
    When SUT-1 is transitioned to "complete" naming a fresh approved review at its current revision with its approval's verdict event
  ...
  Scenario: subtree revision fences history, not just state
    ...
    And SUT-1 has an unspent approved review named, with its current revision and verdict event, on every attempt
  ...
  Scenario: detaching a child cannot leave a stale close fence
    ...
    And completing SUT-50 — naming its approved review, revision, and verdict event — with the previously observed expected_subtree_revision is rejected with a conflict naming the revision, not the review gate
  ...
  Scenario: a blocked descendant holds the parent open
    ...
    When SUT-27 is transitioned to "complete" naming its approved review at its current revision with its approval's verdict event
